...
- **Master password** `fucknabs` → full alliance view, player dropdown VISIBLE.
...
- None. The site is public/static and the Dropbox links are unauthenticated ("anyone with the
  link"). The master password `fucknabs` and roster IDs are read from the workbook, not secrets.
